...
ProgressDialog queryPd,factoryListPd,toFinishPd;
	@Override
...
public void applyWaixiuByHttp(){
	toFinishPd=ProgressDialog.show(WaiXiuApplyTask.this, "处理中", "请稍候...");
	...
	applywaixiuTaskHttpClient.post(applywaixiuUrl, requestparams, new JsonHttpResponseHandler(){
		...
		public void onSuccess(int statusCode, JSONObject response) {
			// TODO 自动生成的方法存根
			super.onSuccess(statusCode, response);
			if(toFinishPd!=null&&toFinishPd.isShowing()){
				toFinishPd.dismiss();
				toFinishPd=null;
			}
			boolean success=false;
		...
		public void onFailure(Throwable e, JSONObject errorResponse) {
			...
			Toast.makeText(WaiXiuApplyTask.this, "申请失败，请重试", Toast.LENGTH_SHORT).show();
		}
